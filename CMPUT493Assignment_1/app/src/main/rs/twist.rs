...
uchar4 __attribute__((kernel)) twist(uchar4 in, uint32_t x, uint32_t y)
{
	...
	if(radius < m_radius)
	{
		...
		hackX = rsClamp(hackX, 0, m_width);
		hackY = rsClamp(hackY, 0, m_height);

		rsDebug("Hax (x, y) : ", hackX, hackY);

		result = *(uchar4*)(rsGetElementAt(m_source, hackX, hackY));
	}
